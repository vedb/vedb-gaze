...
def _match_data(pupil_list, ref_list):
    ...
    matched_binocular_data = closest_matches_binocular(
        ref_list, pupil0, pupil1
    )
    ...
    logger.info(
        f"Collected {len(matched_binocular_data)} binocular calibration data."
    )
...
def closest_matches_binocular(
    ref_pts, pupil0, pupil1, max_dispersion=1 / 15.0
):
...
def _extract_2d_data_monocular(matched_data):
    ...
    cal_data = [
        (*pair["pupil"]["norm_pos"], *pair["ref"]["norm_pos"])
